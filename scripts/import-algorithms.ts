...
const TOPIC_META: Record<string, { category: string; subcategory: string; difficulty: number; tags: string[] }> = {
  // ── ALGEBRA (29) ──────────────────────────────────────────────────────────
...
// ─── Problem Parser ───────────────────────────────────────────────────────────
function parseProblems(content: string) {
  const problems: { name: string; platform: string; link: string; rating: number | null }[] = []
...
async function importAllArticles() {
  ...
  for (const [slug, meta] of Object.entries(TOPIC_META)) {
    ...
    await (prisma as any).algorithmArticle.upsert({
      where: { slug },
      create: {
        slug, title, ...meta, content: rawContent,
        problems: { create: problems }
      },
      update: {
        title, content: rawContent, ...meta,
        problems: { deleteMany: {}, create: problems }
      }
    })
